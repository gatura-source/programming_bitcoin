...
class FiniteElement():
	...
	def __init__(self, element, prime): #initiate the finite field
		if type(element) == int:
		...
		elif type(element) == FiniteElement:
			if element.element >= prime or element.element < 0: #check if element btwn 0 and prime-1
				error = 'Element {} not in the range of the finite field of 0 to {}'\
				.format(element, prime-1)
				raise ValueError(error)
	...
	def __pow__(self, exponent):
		if type(self.element) == int:
			element = (self.element ** exponent) % self.prime
		elif type(self.element) == FiniteElement:
			element = (self.element.element ** exponent) % self.prime
		return self.__class__(element, self.prime)
	...
	def __truediv__(self, other):
		if self.prime != other.prime:
			...
			raise TypeError(exception)
		if type(self.element) == int:
			element = (self.element *pow(other.element, self.prime-2,\
			 self.prime)) % self.prime
		elif type(self.element) == FiniteElement:
			element = (self.element.element *pow(other.element.element, self.prime-2,\
			 self.prime)) % self.prime
		return self.__class__(element, self.prime)
...
class S256Point(Point):#defining the point for the field above
	...
	def SEC(self):
		"returns uncompressed serialized point"
		prefix_byte = b'x04'
